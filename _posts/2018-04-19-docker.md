...
1. Look for `debian package libpng` on Google, which will take you to a page like [this](https://packages.debian.org/search?keywords=libpng). You can click on `stretch` to get more details on what's up with that package in the `stretch` version of Debian.
...
Now, building and running as above, we win!
